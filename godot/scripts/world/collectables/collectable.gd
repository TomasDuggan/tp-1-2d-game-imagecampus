...
@onready var _animation: CollectableAnimation = $Animation
...
func _ready():
	...
	_hurtbox.initialize(self, _config.hp, Hurtbox.DamageFaction.ENEMY, false, _config.get_hit_sfx())
	_hurtbox.destroyed.connect(_on_hurtbox_destroyed, CONNECT_ONE_SHOT)
	_hurtbox.hit.connect(_hit)
	
	_animation.initialize(_config.scale_override, _config.sprite_frames)
	_animation.death_animation_finished.connect(func(): queue_free())
...
func _on_animation_requested(animation_name: String) -> void:
	_animation.animation_requested(animation_name)

func _hit() -> void:
	_animation.play_hit_animation()
