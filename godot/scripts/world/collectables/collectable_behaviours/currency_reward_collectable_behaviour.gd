...
func on_destroyed_by_hero(_source: Hero) -> void:
	...
	var amount_upgraded: int = currency_config.reward_amount + int(UpgradesManager.get_modifier_value(currency_config.type, UpgradesManager.UpgradeId.CURRENCY_GATHER))
