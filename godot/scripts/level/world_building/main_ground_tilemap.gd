...
Lo hice asi para poder usar 'set_cells_terrain_connect' y conectar los rooms dinamicamente.
"""

const MERGE_TERRAIN_ID := 0

var _tile_height: int
var _terrain_set_id: int


func initialize(world_type: World.WorldType) -> void:
	_terrain_set_id = 0 if World.is_miner_world(world_type) else 1
...
func _connect_tilemaps(before_rect: Rect2i) -> void:
	...
	var row_cells: Array[Vector2i] = _get_connecting_band_cells(before_rect, merge_row_y)
	
	if !row_cells.is_empty():
		# Para debug. Muestra las cells conectoras:
		#for cell in row_cells:
			#set_cell(cell, 0, Vector2i(9,1))
		set_cells_terrain_connect(row_cells, _terrain_set_id, MERGE_TERRAIN_ID)

func _get_connecting_band_cells(rect: Rect2i, y: int) -> Array[Vector2i]:
	const BAND_HEIGHT := 3
